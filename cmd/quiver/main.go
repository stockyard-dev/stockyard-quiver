...
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("quiver: %v",err)};defer db.Close();srv:=server.New(db,server.DefaultLimits())
fmt.Printf("\n  Quiver — Self-hosted reading list and learning tracker\n  Dashboard:  http://localhost:%s/ui\n  API:        http://localhost:%s/api\n\n",port,port)
