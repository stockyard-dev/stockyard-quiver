package server
import ("encoding/json";"log";"net/http";"github.com/stockyard-dev/stockyard-quiver/internal/store")
...
s.mux.HandleFunc("GET /api/reading_items",s.list)
s.mux.HandleFunc("POST /api/reading_items",s.create)
s.mux.HandleFunc("GET /api/reading_items/{id}",s.get)
s.mux.HandleFunc("PUT /api/reading_items/{id}",s.update)
s.mux.HandleFunc("DELETE /api/reading_items/{id}",s.del)
s.mux.HandleFunc("GET /api/stats",s.stats)
s.mux.HandleFunc("GET /api/health",s.health)
s.mux.HandleFunc("GET /ui",s.dashboard);s.mux.HandleFunc("GET /ui/",s.dashboard);s.mux.HandleFunc("GET /",s.root);
s.mux.HandleFunc("GET /api/tier",func(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"tier":s.limits.Tier,"upgrade_url":"https://stockyard.dev/quiver/"})})
return s}
...
func(s *Server)list(w http.ResponseWriter,r *http.Request){
    ...
    if q!=""||len(filters)>0{wj(w,200,map[string]any{"reading_items":oe(s.db.Search(q,filters))});return}
    wj(w,200,map[string]any{"reading_items":oe(s.db.List())})
}
func(s *Server)create(w http.ResponseWriter,r *http.Request){if s.limits.MaxItems>0{if s.db.Count()>=s.limits.MaxItems{we(w,402,"Free tier limit reached. Upgrade at https://stockyard.dev/quiver/");return}};var e store.ReadingItem;json.NewDecoder(r.Body).Decode(&e);if e.Title==""{we(w,400,"title required");return};s.db.Create(&e);wj(w,201,s.db.Get(e.ID))}
...
patch.CompletedAt=existing.CompletedAt}
    if patch.Rating==0{patch.Rating=existing.Rating}
...
func(s *Server)health(w http.ResponseWriter,r *http.Request){wj(w,200,map[string]any{"status":"ok","service":"quiver","reading_items":s.db.Count()})}
